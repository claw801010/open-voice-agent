...
|--------------------------------|-------|-------|
| _(none)_ | | **DevExp** — [bootstrap_fresh_dev.sh](scripts/bootstrap_fresh_dev.sh) + [READMEBUILDME §4](READMEBUILDME.md#one-command-local-bootstrap). *Suggested next:* document Windows (PowerShell) parallel or `WSL2`; **MK-01** / **MK-01-RUBRIC** when staffed. |

#### Rolling pass notes (keep short; update when you ship a slice)

| | |
|--|--|
| **This pass (shipped in repo now)** | **bootstrap_fresh_dev.sh** — venv, `api` pip, `api/.env` from example, [docker-compose-local.yaml](docker-compose-local.yaml), [migrate.sh](scripts/migrate.sh), `ui` `npm ci` + `npm test`. Docs: [READMEBUILDME](READMEBUILDME.md#one-command-local-bootstrap), [AGENTS.md](AGENTS.md#local-development). |
| **Next pass (suggested)** | **Windows** companion script or documented `bootstrap_fresh_dev.ps1` slice; `mergeMissingKeysFromDefault` / CallContextEditor Vitest; refresh **http-api** doc screenshots; **MK-01** marketplace / **MK-01-IMPORT** when staffed. |
