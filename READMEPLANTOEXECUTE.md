...
|--------------------------------|-------|-------|
| _(none)_ | | **WE-01-DUALMODE** — Call-context Form **Add missing preset rows** + [collectPresetDotPaths](ui/src/lib/callContextSampleForm.ts). *Suggested next:* **WE-01-DATASTORE-INTEG** or **MK-01**; searchable variable picker if lists grow. |

#### Rolling pass notes (keep short; update when you ship a slice)

| | |
|--|--|
| **This pass (shipped in repo now)** | **Call context Form** — [Add missing preset rows](ui/src/app/tools/[toolUuid]/components/CallContextSampleEditor.tsx); [collectPresetDotPaths](ui/src/lib/callContextSampleForm.ts) / [pathValueMapFromSampleJson](ui/src/lib/callContextSampleForm.ts) + [callContextSampleForm.test.ts](ui/src/lib/callContextSampleForm.test.ts); [http-api.mdx](docs/voice-agent/tools/http-api.mdx). |
| **Next pass (suggested)** | **WE-01-DATASTORE-INTEG** spike or **MK-01**; **Windows** / WSL bootstrap doc; **http-api** screenshots; optional **searchable** variable picker (Combobox) for very long lists. |
