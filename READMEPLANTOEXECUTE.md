...
| **Next pass (suggested)** | **MK-01-RUBRIC:** runbook “book in N turns” steps per **booking_complex** variant (after HTTP tool wired). **WE-01-DATASTORE-INTEG (optional):** cache-hit counters on **`/usage`** or analytics insights. **WE-01-VISUAL-DEPTH:** extend **`ovo-*`** to **`/overview`** / **`/reports`**. Push local commits (`cb8144a`, `7ab749d`, …) when ready. |
...
**Next pass:** **MK-01-RUBRIC** — runbook booking happy paths per variant; **booking stub** for manual QA ([booking-scheduling-stub-local.md](catalog/recipes/booking-scheduling-stub-local.md)). **MK-01-ANALYTICS-VERTICAL** — **`Done`**. **WE-01-DATASTORE-INTEG** — **`Done`** (Redis v1 + **`cache_hit`** on analytics HTTP spans). **WE-01-VOICE-PROFILES** — **`Done`** (GTM PNGs committed).
