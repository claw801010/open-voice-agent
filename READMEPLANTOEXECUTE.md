...
- [x] **GTM capture** — optional **`gtm-we01-http-tool-happy-path.png`** when **`E2E_GTM_HTTP_TOOL_UUID`** is set ([gtm-deck-screenshots.spec.ts](ui/e2e/gtm-deck-screenshots.spec.ts)).
...
| WE-01-DATASTORE-INTEG | Org datastore vs integration-backed HTTP cache (**InProgress** — org draft **GET**/**PUT** + **`/settings`** + HTTP tool **`response_storage_mode`** authoring; runtime cache **off**; [storage model](docs/voice-agent/tools/http-api.mdx#storage-model), [policy](docs/integrations/http-tool-data-policy.mdx), [design](docs/integrations/http-tool-org-datastore-design.mdx); **no runtime cache before 2026-07-01** unless README+design revised) | Pillar 1; Integrations row |
