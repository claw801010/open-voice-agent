...
|--------------------------------|-------|-------|
| _(none)_ | | **WE-01-DUALMODE** — HTTP API variable insert: **caret-safe** replace/append ([templateSnippetInsert.ts](ui/src/components/http/templateSnippetInsert.ts)); **per-tool** custom `{{…}}` localStorage ([page.tsx](ui/src/app/tools/[toolUuid]/page.tsx)). *Suggested next:* **GTM** real **http-api** screenshots + capture runbook; **MK-01** importer golden when code lands; **DATASTORE** admin write path (or defer). |

#### Rolling pass notes (keep short; update when you ship a slice)

| | |
|--|--|
| **This pass (shipped in repo now)** | **WE-01-DUALMODE / Tool API** — Shared **template insert** at caret (URL, JSON, call-context value, parameter **value template**, header **value**): [templateSnippetInsert.ts](ui/src/components/http/templateSnippetInsert.ts), [url-input.tsx](ui/src/components/http/url-input.tsx), [jsonTemplateTextarea.tsx](ui/src/app/tools/[toolUuid]/components/jsonTemplateTextarea.tsx), [CallContextSampleEditor.tsx](ui/src/app/tools/[toolUuid]/components/CallContextSampleEditor.tsx), [parameter-editor.tsx](ui/src/components/http/parameter-editor.tsx), [key-value-editor.tsx](ui/src/components/http/key-value-editor.tsx); insert-mode labels + copy in [HttpApiToolConfig.tsx](ui/src/app/tools/[toolUuid]/components/HttpApiToolConfig.tsx). **Custom flow variables** — `tool-custom-variable-suggestions:{toolUuid}` + migration from legacy global key ([page.tsx](ui/src/app/tools/[toolUuid]/page.tsx)). **Docs** — [http-api.mdx](docs/voice-agent/tools/http-api.mdx) picker insert + per-tool storage. **Tests** — [templateSnippetInsert.test.ts](ui/src/components/http/templateSnippetInsert.test.ts). |
| **Next pass (suggested)** | **GTM** — Authentic **http-api-*.png** + step-by-step capture in [docs/images/README.md](docs/images/README.md); **MK-01** — importer golden vs [n8n-minimal-http-request.json](catalog/fixtures/n8n-minimal-http-request.json); **DATASTORE** — admin **PUT** policy stub or explicit “no write API this quarter”. |
...
- [ ] **Adapter code** (Make / n8n / Zapier / skills): tests + update READMEMARKETPLACEPLANNING §5 “Shipped experiments” when a concrete importer lands.
...
**Status:** `InProgress` (documentation + extension-point stub in repo; **no** cache env flags until org policy ships; authoring / admin / full runtime cache not started)  
...
- [x] **Runtime (read stub)** — `GET /api/v1/organizations/http-integration-cache-policy` returns `cache_enabled: false`, `deferral_not_before`, `implementation_status` for the selected org ([organization.py](api/routes/organization.py), [http_tool_cache_policy.py](api/services/workflow/tools/http_tool_cache_policy.py)); [test_http_integration_cache_policy_route.py](api/tests/test_http_integration_cache_policy_route.py); HTTP tool **UI** surfaces the same payload when available ([page.tsx](ui/src/app/tools/[toolUuid]/page.tsx)).
...
| WE-01-DATASTORE-INTEG | Org datastore default vs integration-backed HTTP cache (**InProgress** — `GET …/http-integration-cache-policy` read stub; [storage model](docs/voice-agent/tools/http-api.mdx#storage-model), [policy](docs/integrations/http-tool-data-policy.mdx), [design](docs/integrations/http-tool-org-datastore-design.mdx); **no runtime cache before 2026-07-01** unless README+design revised; env hook **retracted**) | Pillar 1; Integrations row |
