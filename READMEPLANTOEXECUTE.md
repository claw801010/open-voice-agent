...
|--------------------------------|-------|-------|
| _(none)_ | | **WE-01-DATASTORE-INTEG** — [Org datastore design](docs/integrations/http-tool-org-datastore-design.mdx) + policy doc fix; **MK-01** [n8n manual import spike](catalog/import-adapter-n8n-spike.md); **WE-01-DUALMODE** HTTP pickers always show four groups + **Use app default** on call-context rows. *Suggested next:* DATASTORE **implementation** (admin policy CRUD + runtime) or keep design-only until staffed; **MK-01** n8n **importer code** or next adapter; **GTM** — real **docs/images/http-api-*.png** screenshots. |

#### Rolling pass notes (keep short; update when you ship a slice)

| | |
|--|--|
| **This pass (shipped in repo now)** | **WE-01-DATASTORE-INTEG** — [http-tool-org-datastore-design.mdx](docs/integrations/http-tool-org-datastore-design.mdx) (policy + persistence design); [http-tool-data-policy.mdx](docs/integrations/http-tool-data-policy.mdx) summary aligned (no cache env). **MK-01-IMPORT-OPTIONS** — [import-adapter-n8n-spike.md](catalog/import-adapter-n8n-spike.md), [IMPORT_ADAPTERS_SPIKE.md](catalog/IMPORT_ADAPTERS_SPIKE.md) first-target row, [catalog/README.md](catalog/README.md). **WE-01-DUALMODE / Tool API** — variable groups always visible (empty-group hint); call-context Form **Use app default**; picker filter keeps empty groups only when not searching; tooltips + banner copy. |
| **Next pass (suggested)** | **WE-01-DATASTORE-INTEG** — admin policy API/UI + runtime read path behind a flag, or document **won’t ship** quarter; **MK-01-IMPORT** — importer code for n8n subset **or** Zapier/Make spike doc parity; **GTM** — replace **docs/images/http-api-*.png** with real captures. |
...
| WE-01-DATASTORE-INTEG | Org datastore default vs integration-backed HTTP cache (**InProgress** — [storage model](docs/voice-agent/tools/http-api.mdx#storage-model), [policy](docs/integrations/http-tool-data-policy.mdx), [design](docs/integrations/http-tool-org-datastore-design.mdx); premature env hook **retracted**) | Pillar 1; Integrations row |
