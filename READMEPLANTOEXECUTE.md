...
|--------------------------------|-------|-------|
| _(none)_ | | **WE-01-DUALMODE** — body template **Add missing parameter keys**; **MK-01** n8n script **`--http-hints`**; **DATASTORE** cache deferral **2026-07-01** in design doc + README. *Suggested next:* DATASTORE **read** policy stub API (optional) or move deferral date after review; **MK-01** golden n8n fixture in repo + CI one-liner; **GTM** — real **http-api** screenshots. |

#### Rolling pass notes (keep short; update when you ship a slice)

| | |
|--|--|
| **This pass (shipped in repo now)** | **WE-01-DUALMODE / Tool API** — [seedBodyTemplateJsonFromParameters](ui/src/lib/callContextSampleForm.ts) + **Add missing parameter keys** on body template (Simple + Advanced); [HttpApiToolConfig.tsx](ui/src/app/tools/[toolUuid]/components/HttpApiToolConfig.tsx), [page.tsx](ui/src/app/tools/[toolUuid]/page.tsx). **MK-01-IMPORT-OPTIONS** — [validate-n8n-workflow-export.mjs](catalog/scripts/validate-n8n-workflow-export.mjs) **`--http-hints`** JSON. **WE-01-DATASTORE-INTEG** — README acceptance + design **Fork posture** **2026-07-01** deferral. |
| **Next pass (suggested)** | **WE-01-DATASTORE-INTEG** — `GET …/http-integration-cache-policy` stub returning `{ cache_enabled: false }` (auth + org scope) **or** revise deferral date; **MK-01** — commit **one** small `catalog/fixtures/n8n-*.json` + CI `node … --http-hints`; **GTM** — **docs/images/http-api-*.png** real screenshots. |
