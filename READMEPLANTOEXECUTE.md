...
|--------------------------------|-------|-------|
| _(none)_ | | **WE-01-DUALMODE** — Picker **fallback hints** for custom / live / “From your flow” rows + filter ([contextVariableTemplates.ts](ui/src/constants/contextVariableTemplates.ts), [grouped-string-option-picker.tsx](ui/src/components/http/grouped-string-option-picker.tsx)). *Suggested next:* **GTM** real **http-api** screenshots + capture runbook; **MK-01** importer golden when code lands; **DATASTORE** admin write path (or defer). |

#### Rolling pass notes (keep short; update when you ship a slice)

| | |
|--|--|
| **This pass (shipped in repo now)** | **WE-01-DUALMODE / Tool API** — `groupedPickerFallbackHint` / `resolveGroupedPickerRowHint` / `buildGroupedPickerFilterSubtitleLookup` so **Custom**, **Live**, and **From your flow** rows always show a filterable hint ([contextVariableTemplates.ts](ui/src/constants/contextVariableTemplates.ts), [grouped-string-option-picker.tsx](ui/src/components/http/grouped-string-option-picker.tsx), tests). **Docs** — [http-api.mdx](docs/voice-agent/tools/http-api.mdx); [HttpApiToolConfig.tsx](ui/src/app/tools/[toolUuid]/components/HttpApiToolConfig.tsx) explainer. |
