...
|--------------------------------|-------|-------|
| _(none)_ | | **WE-01-DUALMODE** — Raw Code tab **variable picker**; **per-tool** test call-context JSON + **Form / JSON** tab localStorage ([page.tsx](ui/src/app/tools/[toolUuid]/page.tsx), [HttpApiToolConfig.tsx](ui/src/app/tools/[toolUuid]/components/HttpApiToolConfig.tsx)). *Suggested next:* **GTM** real **http-api** screenshots + capture runbook; **MK-01** importer golden when code lands; **DATASTORE** admin write path (or defer). |

#### Rolling pass notes (keep short; update when you ship a slice)

| | |
|--|--|
| **This pass (shipped in repo now)** | **WE-01-DUALMODE / Tool API** — **Raw Code** tab: [JsonTemplateTextarea](ui/src/app/tools/[toolUuid]/components/jsonTemplateTextarea.tsx) + grouped pickers ([HttpApiToolConfig.tsx](ui/src/app/tools/[toolUuid]/components/HttpApiToolConfig.tsx)); `spellCheck` prop on textarea. **Test call context** — `tool-http-call-context-json:{toolUuid}` + legacy migration ([page.tsx](ui/src/app/tools/[toolUuid]/page.tsx)); **Form / JSON** tab scoped via [CallContextSampleEditor.tsx](ui/src/app/tools/[toolUuid]/components/CallContextSampleEditor.tsx) `storageScopeId`. **Docs** — [http-api.mdx](docs/voice-agent/tools/http-api.mdx) raw + per-tool sample keys. |
