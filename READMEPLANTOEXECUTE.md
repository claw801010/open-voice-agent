...
|--------------------------------|-------|-------|
| _(none)_ | | **WE-01-DATASTORE-INTEG** — Retracted premature cache **env** hook; **MK-01** native JSON [import playbook](catalog/import-packaged-workflow-json.md); **http-api** [placeholder PNGs](docs/images/). *Suggested next:* DATASTORE **org policy + persistence** design doc or implementation spike; **MK-01** external adapter code; swap **docs/images/http-api-*.png** for real screenshots. |

#### Rolling pass notes (keep short; update when you ship a slice)

| | |
|--|--|
| **This pass (shipped in repo now)** | **WE-01-DATASTORE-INTEG** — Retracted `HTTP_TOOL_INTEGRATION_CACHE_ENABLED` + warning; [http_tool_cache_policy.py](api/services/workflow/tools/http_tool_cache_policy.py) doc-only; [http-tool-data-policy.mdx](docs/integrations/http-tool-data-policy.mdx). **MK-01-IMPORT-OPTIONS** — [import-packaged-workflow-json.md](catalog/import-packaged-workflow-json.md), spike **owner** row, [catalog/README.md](catalog/README.md). **Docs** — [docs/images/](docs/images/) `http-api-*.png` placeholders + [http-api.mdx](docs/voice-agent/tools/http-api.mdx) embeds. |
| **Next pass (suggested)** | **WE-01-DATASTORE-INTEG** — org-level policy schema + cache read/write (or formal “won’t do” decision in README); **MK-01-IMPORT** — first external adapter spike or close to **NotStarted**; **GTM** — replace placeholder PNGs with real **http-api** screenshots. |
...
| WE-01-DATASTORE-INTEG | Org datastore default vs integration-backed HTTP cache (**InProgress** — [docs](docs/voice-agent/tools/http-api.mdx#storage-model), [policy](docs/integrations/http-tool-data-policy.mdx); premature env hook **retracted**) | Pillar 1; Integrations row |
