...
|--------------------------------|-------|-------|
| _(none)_ | | **WE-01-DUALMODE** — Call-context **Preset path** autofill + grouped picker **header tooltips** ([mergePresetPathPick](ui/src/lib/callContextSampleForm.ts), [GROUPED_PICKER_BUILTIN_HEADER_TOOLTIPS](ui/src/constants/contextVariableTemplates.ts)). *Suggested next:* DATASTORE cache implementation or **MK-01** import code; **http-api** PNG assets. |

#### Rolling pass notes (keep short; update when you ship a slice)

| | |
|--|--|
| **This pass (shipped in repo now)** | **WE-01-DUALMODE / HTTP tool API** — [mergePresetPathPick](ui/src/lib/callContextSampleForm.ts) + Form **Preset path** autofill empty values from default sample; [GroupedStringOptionPicker](ui/src/components/http/grouped-string-option-picker.tsx) native `title` via [GROUPED_PICKER_BUILTIN_HEADER_TOOLTIPS](ui/src/constants/contextVariableTemplates.ts); [CallContextSampleEditor](ui/src/app/tools/[toolUuid]/components/CallContextSampleEditor.tsx), [HttpApiToolConfig](ui/src/app/tools/[toolUuid]/components/HttpApiToolConfig.tsx), [http-api.mdx](docs/voice-agent/tools/http-api.mdx); [IMPORT_ADAPTERS_SPIKE.md](catalog/IMPORT_ADAPTERS_SPIKE.md) suggested first target. |
| **Next pass (suggested)** | **WE-01-DATASTORE-INTEG** — real cache read/write + org policy + admin UI; **MK-01-IMPORT** — owner + ship spike/code or revert package to **NotStarted**; **http-api** — add three screenshot PNGs to the docs site. |
