...
| **Next pass (suggested)** | ElevenLabs voice preview regen when `ELEVENLABS_API_KEY` is set (`python scripts/generate_catalog_voice_preview_audio.py`). **GTM deck live capture** — done (17 PNGs under [docs/images/](docs/images/); [gtm_capture_deck.sh](scripts/gtm_capture_deck.sh), [check_gtm_capture_prereqs.sh](scripts/check_gtm_capture_prereqs.sh)). |
...
**Next pass:** **MK-01-CATALOG-INSURANCE** — **`Done`** (seed pack). **Insurance PREBUILD tail:** quote qual / claims lookup variants. **WE-01** polish optional.
