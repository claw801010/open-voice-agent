...
**Status values:** `NotStarted` | `InProgress` | `Incomplete` | `Blocked` | `Done` (`Incomplete` = shipped in working tree but not merged/released; mirror `Done` into READMENEWRELEASES only after merge).
...
**Next pass:** **MK-01-CATALOG-INSURANCE** — **`Done`** (seed pack). **Insurance PREBUILD tail:** **`Done`**. **WE-01** polish optional.
...
- [x] **`buyer-demo-defaults.json`** / **`buyer-demo-hints.json`** + **`gen_buyer_demo_shortcuts.py --check`** + **`buyer-demo-*.sh`** (10 verticals).
- [x] **`seed_gtm_all_buyer_demo_calls.py`** / **`seed_gtm_all_buyer_workflows.py`** → Playwright **`E2E_GTM_*`** env map ([buyerDemoSeededCalls.ts](ui/src/lib/catalog/buyerDemoSeededCalls.ts)).
- [x] **`verify_mk01_buyer_shipped.sh`**, **`gtm_buyer_demo_pack.sh`**, **`buyer-demo-gtm-day.md`** SE runbook.
- [x] **42/42** Playwright GTM deck capture; **`LocalEhrSection`** OSS auth wait; smoke script regression guards.
- [x] CI — [analytics-buyer-demo-calls.spec.ts](ui/e2e/analytics-buyer-demo-calls.spec.ts), [api-pytest-usage-rollup.yml](.github/workflows/api-pytest-usage-rollup.yml) buyer matrix units.
...
- [ ] Stage **23+** untracked files and commit (or land split PRs 1–4).
- [ ] `./scripts/verify_mk01_buyer_shipped.sh` green on the branch.
- [ ] Push + open PR; mirror into **READMENEWRELEASES** on merge.

**Key files:** [prepare_mk01_pr.sh](scripts/prepare_mk01_pr.sh), [DOCS.md](DOCS.md), [docs/images/gtm-mk01-*.png](docs/images/).

---

### MK-01-VOICE-SPOKEN — ElevenLabs catalog voice previews

**Status:** `Incomplete`

**Goal:** Replace silent **`catalog/voice-previews/*.wav`** placeholders with spoken ElevenLabs samples for marketplace **Preview voice**.
...
**Key files:** [regen_catalog_voice_previews.sh](scripts/regen_catalog_voice_previews.sh), [catalog/voice-previews/README.md](catalog/voice-previews/README.md).
...
### MK-01-CATALOG-HOSPITALITY — Fifth vertical (Hospitality / travel)

**Status:** `Done`

**Goal:** Seed READMEPLANNING §6 **Hospitality / travel** row: concierge FAQ + **`booking_complex`** reservation modify variant.

**Shipped:**

- [x] Pack **`hospitality-travel-concierge`** **1.0.0** in [vertical-packs.json](catalog/vertical-packs.json) with **`roadmap_motions`** (waiver, loyalty upsell).
- [x] Graphs [hospitality-travel-concierge.json](catalog/packaged-workflows/hospitality-travel-concierge.json), [hospitality-travel-booking-complex.json](catalog/packaged-workflows/hospitality-travel-booking-complex.json).
- [x] Runbook [hospitality-travel-concierge.md](runbooks/hospitality-travel-concierge.md); CI install tests.

---

### MK-01-CATALOG-FINANCIAL — Sixth vertical (Financial services)

**Status:** `Done`

**Goal:** Seed READMEPLANNING §6 **Financial services** row: card/branch FAQ + **`booking_complex`** branch appointment variant.

**Shipped:**

- [x] Pack **`financial-services-banking-faq`** **1.0.0** in [vertical-packs.json](catalog/vertical-packs.json) with **`roadmap_motions`** (balance lookup, card block).
- [x] Graphs [financial-services-banking-faq.json](catalog/packaged-workflows/financial-services-banking-faq.json), [financial-services-booking-complex.json](catalog/packaged-workflows/financial-services-booking-complex.json).
- [x] Runbook [financial-services-banking-faq.md](runbooks/financial-services-banking-faq.md); CI install tests.

---

### MK-01-CATALOG-SMB — Seventh vertical (SMB / franchises)

**Status:** `Done`

**Goal:** Seed READMEPLANNING §6 **SMB / franchises** row: multi-location FAQ + **`booking_complex`** lead callback variant.

**Shipped:**

- [x] Pack **`smb-franchise-location-faq`** **1.0.0** in [vertical-packs.json](catalog/vertical-packs.json) with **`roadmap_motions`** (location router, CRM lead capture).
- [x] Graphs [smb-franchise-location-faq.json](catalog/packaged-workflows/smb-franchise-location-faq.json), [smb-franchise-booking-complex.json](catalog/packaged-workflows/smb-franchise-booking-complex.json).
- [x] Runbook [smb-franchise-location-faq.md](runbooks/smb-franchise-location-faq.md); CI install tests.

---

### MK-01-CATALOG-INSURANCE — Fourth vertical (Insurance FNOL)

**Status:** `Done`

**Goal:** Seed READMEPLANNING §6 **Insurance** row: simple FNOL/policy FAQ graph + **`booking_complex`** adjuster callback variant.

**Shipped:**

- [x] Pack **`insurance-fnol-faq`** **1.0.0** in [vertical-packs.json](catalog/vertical-packs.json) with **`roadmap_motions`** (quote qual, claims lookup).
- [x] Graphs [insurance-fnol-faq.json](catalog/packaged-workflows/insurance-fnol-faq.json), [insurance-fnol-booking-complex.json](catalog/packaged-workflows/insurance-fnol-booking-complex.json).
- [x] Runbook [insurance-fnol-faq.md](runbooks/insurance-fnol-faq.md) — happy path + booking-complex + roadmap section.
- [x] CI — catalog tests + [test_install_from_catalog_insurance_default](api/tests/test_install_from_catalog_routes.py), [test_install_from_catalog_insurance_booking_complex](api/tests/test_install_from_catalog_routes.py); [analyticsVerticalHttpHints.ts](ui/src/lib/analyticsVerticalHttpHints.ts).

---

### MK-01-PREBUILD-COMPLETE — All revenue motions shipped

**Status:** `Done`

**Goal:** Close the PREBUILD arc: all three vertical packs have empty **`roadmap_motions`**, every complex variant has runbook + install API + analytics proof, and GTM has a single demo matrix.

**Shipped:**

- [x] **`roadmap_motions: []`** on healthcare, retail, B2B ([vertical-packs.json](catalog/vertical-packs.json) **1.4.0**).
- [x] **CI** — [test_prebuild_roadmap_motions_all_shipped](api/tests/test_vertical_packs_catalog.py), [test_prebuild_complex_variants_present](api/tests/test_vertical_packs_catalog.py).
- [x] **GTM matrix** — [prebuild-vertical-demo-matrix.md](catalog/recipes/prebuild-vertical-demo-matrix.md).
- [x] **Playwright** — catalog UI install with **`collections_complex`** variant ([catalog-marketplace.spec.ts](ui/e2e/catalog-marketplace.spec.ts)).
- [x] **PREBUILD checklist** — [PREBUILD_VERTICAL_ROADMAP.md](catalog/PREBUILD_VERTICAL_ROADMAP.md) marketing gate ticked.
...
**Goal:** Tune how agents **sound** (tone, behavior, authenticity, professional/extended/multilingual fillers, breath pauses, ElevenLabs stability/similarity) without editing prompts per agent. Built-in presets, **seven MK-01 vertical profiles**, org default, custom profiles (clone/edit/tags), workflow override, runtime TTS + LLM delivery block on live calls.
...
- [x] **Vertical built-ins** — `vertical_healthcare` … `vertical_smb` with industry tone/behavior/filler defaults ([vertical_presets.py](api/services/voice/vertical_presets.py)); **`recommended_voice_profile_id`** on catalog packs; install sets **`voice_profile_id`** ([workflow.py](api/routes/workflow.py)).
...
| MK-01-CATALOG-HOSPITALITY | Fifth vertical — Hospitality / travel seed (**`Done`**) | §6 Hospitality row |
| MK-01-CATALOG-FINANCIAL | Sixth vertical — Financial services seed (**`Done`**) | §6 Financial services row |
| MK-01-CATALOG-SMB | Seventh vertical — SMB / franchises seed (**`Done`**) | §6 SMB row |
...
| 2026-05-25 | **MK-01 depth (vertical voice profiles)** — **`vertical_*`** built-ins + tone/behavior/extended/multilingual fillers; catalog **`recommended_voice_profile_id`**; install binds **`voice_profile_id`**. |
| 2026-05-25 | **MK-01-PREBUILD (SMB CRM lead capture)** — **`lead_capture_complex`**; all **seven** packs empty **`roadmap_motions`**. |
| 2026-05-25 | **MK-01-PREBUILD (SMB location router)** — **`location_router_complex`** + [smb-franchise-location-router-complex.json](catalog/packaged-workflows/smb-franchise-location-router-complex.json); SMB pack **1.1.0**. |
| 2026-05-25 | **MK-01-CATALOG-SMB** — Seventh vertical **`smb-franchise-location-faq`** (multi-location FAQ + **`booking_complex`** **`schedule_lead_callback`**). |
| 2026-05-25 | **MK-01-PREBUILD (financial card block)** — **`card_block_complex`** + [financial-services-card-block-complex.json](catalog/packaged-workflows/financial-services-card-block-complex.json); all **six** vertical packs have empty **`roadmap_motions`**. |
| 2026-05-25 | **MK-01-PREBUILD (financial balance lookup)** — **`balance_lookup_complex`** + [financial-services-balance-lookup-complex.json](catalog/packaged-workflows/financial-services-balance-lookup-complex.json); financial pack **1.1.0**. |
| 2026-05-25 | **MK-01-CATALOG-FINANCIAL** — Sixth vertical **`financial-services-banking-faq`** (card/branch FAQ + **`booking_complex`** **`schedule_branch_appointment`**). |
| 2026-05-25 | **MK-01-PREBUILD (hospitality loyalty upsell)** — **`upsell_complex`** + [hospitality-travel-upsell-complex.json](catalog/packaged-workflows/hospitality-travel-upsell-complex.json); all **five** vertical packs have empty **`roadmap_motions`**. |
| 2026-05-25 | **MK-01-PREBUILD (hospitality waiver)** — **`waiver_complex`** + [hospitality-travel-waiver-complex.json](catalog/packaged-workflows/hospitality-travel-waiver-complex.json); hospitality pack **1.1.0**. |
| 2026-05-25 | **MK-01-CATALOG-HOSPITALITY** — Fifth vertical **`hospitality-travel-concierge`** (concierge FAQ + **`booking_complex`** **`modify_reservation`**). |
| 2026-05-25 | **MK-01-PREBUILD (insurance claims lookup)** — **`claims_lookup_complex`** + [insurance-fnol-claims-lookup-complex.json](catalog/packaged-workflows/insurance-fnol-claims-lookup-complex.json); all **four** vertical packs have empty **`roadmap_motions`**. |
| 2026-05-25 | **MK-01-PREBUILD (insurance quote intent)** — **`quote_complex`** + [insurance-fnol-quote-complex.json](catalog/packaged-workflows/insurance-fnol-quote-complex.json); insurance pack **1.1.0**. |
| 2026-05-25 | **MK-01-CATALOG-INSURANCE** — Fourth vertical **`insurance-fnol-faq`** (FNOL FAQ + **`booking_complex`** adjuster callback); runbook + install API tests. |
| 2026-05-23 | **MK-01-PREBUILD-COMPLETE** — All packs **`roadmap_motions: []`**; [prebuild-vertical-demo-matrix.md](catalog/recipes/prebuild-vertical-demo-matrix.md); CI [test_prebuild_roadmap_motions_all_shipped](api/tests/test_vertical_packs_catalog.py); Playwright **collections_complex** variant install ([catalog-marketplace.spec.ts](ui/e2e/catalog-marketplace.spec.ts)). |
| 2026-05-23 | **MK-01-PREBUILD (retail collections)** — **`collections_complex`** + [retail-wismo-collections-complex.json](catalog/packaged-workflows/retail-wismo-collections-complex.json); all three vertical packs have empty **`roadmap_motions`**. |
| 2026-05-23 | **MK-01-PREBUILD (healthcare concierge)** — **`concierge_complex`** + [healthcare-triage-concierge-complex.json](catalog/packaged-workflows/healthcare-triage-concierge-complex.json); healthcare **`roadmap_motions`** complete. |
| 2026-05-23 | **MK-01-PREBUILD (B2B trial→paid conversion)** — **`conversion_complex`** + [b2b-trial-conversion-complex.json](catalog/packaged-workflows/b2b-trial-conversion-complex.json); B2B **`roadmap_motions`** complete. |
| 2026-05-23 | **MK-01-PREBUILD (B2B renewal / QBR)** — **`renewal_complex`** + [b2b-trial-renewal-complex.json](catalog/packaged-workflows/b2b-trial-renewal-complex.json); runbook happy path; install API test. |
| 2026-05-23 | **MK-01-PREBUILD (retail paid upsell)** — **`upsell_complex`** + [retail-wismo-upsell-complex.json](catalog/packaged-workflows/retail-wismo-upsell-complex.json); runbook happy path; install API test. |
...
| 2026-05-25 | **MK-01-CATALOG-HR (tenth vertical)** — Pack **`hr-staffing-recruiting-faq`** **1.2.0** + PREBUILD variants (interview scheduling, application status, confirm/reschedule); **`builtin:vertical_hr`**; recipe-aligned local scheduling for **`schedule_interview`**. |
| 2026-05-25 | **MK-01 all-in-one local scheduling** — Built-in calendar API (`/api/v1/local-scheduling`): JSON persistence, open slots, `.ics` invites, install-from-catalog auto-wires **`scheduling_api_base_url`**; recipe [local-scheduling-all-in-one.md](catalog/recipes/local-scheduling-all-in-one.md). **Analytics** — **`cache_hit`** on HTTP tool spans ([call_intel.py](api/services/analytics/call_intel.py)). |
