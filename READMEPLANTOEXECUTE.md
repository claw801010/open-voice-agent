...
|--------------------------------|-------|-------|
| _(none)_ | | **WE-01-DUALMODE** — Variable picker **hints** + filter-by-hint; **per-tool** **test payload** localStorage ([contextVariableTemplates.ts](ui/src/constants/contextVariableTemplates.ts), [grouped-string-option-picker.tsx](ui/src/components/http/grouped-string-option-picker.tsx), [page.tsx](ui/src/app/tools/[toolUuid]/page.tsx)). *Suggested next:* **GTM** real **http-api** screenshots + capture runbook; **MK-01** importer golden when code lands; **DATASTORE** admin write path (or defer). |

#### Rolling pass notes (keep short; update when you ship a slice)

| | |
|--|--|
| **This pass (shipped in repo now)** | **WE-01-DUALMODE / Tool API** — Picker **subtitles** for built-in `{{…}}` + call-context dot paths; filter matches hint text ([contextVariableTemplates.ts](ui/src/constants/contextVariableTemplates.ts), [grouped-string-option-picker.tsx](ui/src/components/http/grouped-string-option-picker.tsx), [contextVariableTemplates.test.ts](ui/src/constants/contextVariableTemplates.test.ts)). **Test payload** — `tool-http-test-payload:{toolUuid}` debounced save ([page.tsx](ui/src/app/tools/[toolUuid]/page.tsx), [HttpApiToolConfig.tsx](ui/src/app/tools/[toolUuid]/components/HttpApiToolConfig.tsx)). **Docs** — [http-api.mdx](docs/voice-agent/tools/http-api.mdx). |
