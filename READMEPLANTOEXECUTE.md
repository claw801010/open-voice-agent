...
|--------------------------------|-------|-------|
| _(none)_ | | **WE-01-DUALMODE** — call-context **JSON** tab merge + HTTP tool **integration cache** banner from policy API; **MK-01** Vitest runs n8n validator on fixture. *Suggested next:* **GTM** real **http-api** screenshots + capture runbook; **MK-01** importer golden when code lands; **DATASTORE** admin write path (or defer). |

#### Rolling pass notes (keep short; update when you ship a slice)

| | |
|--|--|
| **This pass (shipped in repo now)** | **WE-01-DUALMODE / Tool API** — Call context **JSON** tab **Add missing sample keys (JSON)** ([CallContextSampleEditor.tsx](ui/src/app/tools/[toolUuid]/components/CallContextSampleEditor.tsx)); HTTP tool **integration cache** banner from `GET …/http-integration-cache-policy` ([page.tsx](ui/src/app/tools/[toolUuid]/page.tsx), [HttpApiToolConfig.tsx](ui/src/app/tools/[toolUuid]/components/HttpApiToolConfig.tsx)). **MK-01** — [n8nFixtureValidate.test.ts](ui/src/lib/n8nFixtureValidate.test.ts) + [ui-vitest.yml](.github/workflows/ui-vitest.yml) `catalog/` paths. **Docs** — [http-api.mdx](docs/voice-agent/tools/http-api.mdx) call-context JSON note. |
