...
|--------------------------------|-------|-------|
| _(none)_ | | **DATASTORE** `GET …/http-integration-cache-policy`; **MK-01** fixture + **catalog-n8n-validate** CI; **GTM** 1280×720 **http-api** PNGs + `gen_http_api_doc_pngs.py`. *Suggested next:* HTTP tool **UI** optional fetch of policy; **MK-01** importer vs fixture; **GTM** — **authentic** PNG captures. |

#### Rolling pass notes (keep short; update when you ship a slice)

| | |
|--|--|
| **This pass (shipped in repo now)** | **WE-01-DATASTORE-INTEG** — `GET /api/v1/organizations/http-integration-cache-policy` ([organization.py](api/routes/organization.py)); pytest + [api-pytest-usage-rollup.yml](.github/workflows/api-pytest-usage-rollup.yml). **MK-01-IMPORT-OPTIONS** — [catalog/fixtures/n8n-minimal-http-request.json](catalog/fixtures/n8n-minimal-http-request.json), [catalog-n8n-validate.yml](.github/workflows/catalog-n8n-validate.yml). **GTM** — [scripts/gen_http_api_doc_pngs.py](scripts/gen_http_api_doc_pngs.py), **1280×720** [docs/images/http-api-*.png](docs/images/), [http-api.mdx](docs/voice-agent/tools/http-api.mdx), [DOCS.md](DOCS.md). |
| **Next pass (suggested)** | **WE-01-DATASTORE-INTEG** — HTTP tool **optional** policy banner from `GET …/http-integration-cache-policy`; **MK-01** — importer golden test; **GTM** — **real** dashboard **http-api-*.png** + capture steps in [docs/images/README.md](docs/images/README.md). |
...
- [x] **Runtime (read stub)** — `GET /api/v1/organizations/http-integration-cache-policy` returns `cache_enabled: false`, `deferral_not_before`, `implementation_status` for the selected org ([organization.py](api/routes/organization.py), [http_tool_cache_policy.py](api/services/workflow/tools/http_tool_cache_policy.py)); [test_http_integration_cache_policy_route.py](api/tests/test_http_integration_cache_policy_route.py).
