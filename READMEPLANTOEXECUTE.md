...
|--------------------------------|-------|-------|
| _(none)_ | | **WE-01-DUALMODE** — test payload **Add missing parameter keys**; **MK-01** `validate-n8n-workflow-export.mjs`; **DATASTORE** design **Fork posture** note. *Suggested next:* DATASTORE **admin policy API stub** or calendar **won’t ship** line; **MK-01** parse n8n HTTP node → suggested tool JSON (still manual apply); **GTM** — real **http-api** screenshots. |

#### Rolling pass notes (keep short; update when you ship a slice)

| | |
|--|--|
| **This pass (shipped in repo now)** | **WE-01-DUALMODE / Tool API** — [seedTestPayloadJsonFromParameters](ui/src/lib/callContextSampleForm.ts) + **Add missing parameter keys** on Simple + Advanced test JSON ([page.tsx](ui/src/app/tools/[toolUuid]/page.tsx), [HttpApiToolConfig.tsx](ui/src/app/tools/[toolUuid]/components/HttpApiToolConfig.tsx)); Vitest. **MK-01-IMPORT-OPTIONS** — [catalog/scripts/validate-n8n-workflow-export.mjs](catalog/scripts/validate-n8n-workflow-export.mjs); spike + [catalog/README.md](catalog/README.md). **WE-01-DATASTORE-INTEG** — [design doc](docs/integrations/http-tool-org-datastore-design.mdx) **Fork posture** subsection. |
| **Next pass (suggested)** | **WE-01-DATASTORE-INTEG** — org policy **read** API (feature-flagged) + empty admin UI shell, or explicit **won’t ship before (date)** in README + design doc; **MK-01** — extend n8n script to **list HTTP Request** nodes + echo mapping hints JSON; **GTM** — capture **docs/images/http-api-*.png**. |
