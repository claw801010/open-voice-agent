...
### HTTP tools and data residency

<Card title="HTTP tools — org data and cache policy" href="/integrations/http-tool-data-policy">
  Org-local tool definitions, runtime `{{…}}` context, browser-only test samples, and the roadmap for optional integration response cache (**WE-01-DATASTORE-INTEG**)
</Card>

<Card title="HTTP tool — org datastore design" href="/integrations/http-tool-org-datastore-design">
  Proposed policy schema, storage boundaries, and rollout gates before cache code ships (design doc)
</Card>
