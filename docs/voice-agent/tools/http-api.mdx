...
- **Templated URLs:** you may include `{{path.to.value}}` in the path or query, for example `https://api.example.com/orders/{{order_id}}`. The editor accepts these while you are authoring; the URL is resolved at runtime. See [Template resolution order](#template-resolution-order) below.
