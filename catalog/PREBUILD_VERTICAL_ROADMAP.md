...
| [public-sector-civic-services-faq](vertical-packs.json) | Public sector / government | **Simple** + **complex** booking, permit status, language router graphs | **Case callback**, **permit lookup**, **language routing** HTTP tools |
| [hr-staffing-recruiting-faq](vertical-packs.json) | HR / staffing | **Simple** + **complex** booking, application status, interview confirm graphs | **Interview scheduling** (local calendar + `.ics`), **ATS status lookup**, **confirm/reschedule** HTTP tools |
...
| `public-sector-civic-services-faq` | Case worker callback scheduling | Self-serve civic follow-up | **Shipped** — **`booking_complex`** + **`schedule_civic_callback`** + runbook happy path |
| `public-sector-civic-services-faq` | Permit status lookup | Real-time permit records | **Shipped** — **`permit_status_complex`** + **`lookup_permit_status`** + runbook happy path |
| `public-sector-civic-services-faq` | Multilingual routing | Language-appropriate handoff | **Shipped** — **`language_router_complex`** + **`route_by_language`** + runbook happy path |
