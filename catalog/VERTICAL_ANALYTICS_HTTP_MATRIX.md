...
| `smb-franchise-location-faq` | Multi-location FAQ → **lead callback**; **location router**; CRM **roadmap** | `schedule_lead_callback`, `route_call_to_location` (shipped); `capture_lead_intent` (roadmap) | `scheduling_api_base_url`, `locations_api_base_url`, `routing_policy_code`, `brand_name`, `default_location_code`, `location_directory_url`, `preferred_callback_window_hours`, `support_phone` | **`catalog_variant_id`** = `booking_complex` or `location_router_complex`; **`tool_name`** filter + call detail `mapped_data` |
