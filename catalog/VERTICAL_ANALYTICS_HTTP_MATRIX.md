...
| `smb-franchise-location-faq` | Multi-location FAQ → **lead callback**; location router / CRM **roadmap** | `schedule_lead_callback` (shipped); `route_call_to_location`, `capture_lead_intent` (roadmap) | `scheduling_api_base_url`, `brand_name`, `default_location_code`, `location_directory_url`, `preferred_callback_window_hours`, `support_phone` | **`catalog_variant_id`** = `booking_complex`; **`tool_name`** filter + call detail `mapped_data` |
