...
| `insurance-fnol-faq` | FNOL guidance → **adjuster callback** | `schedule_adjuster_callback`, claims lookup tools (roadmap) | `scheduling_api_base_url`, `carrier_name`, `line_of_business`, `claims_portal_url`, `preferred_callback_window_hours` | **`catalog_variant_id=booking_complex`** + **`tool_name=schedule_adjuster_callback`**; call detail `mapped_data` |
