...
| `hospitality-travel-concierge` | Concierge FAQ → **modify reservation** | `modify_reservation`, CRS upsell tools (roadmap) | `pms_api_base_url`, `property_name`, `confirmation_prefix`, `default_room_type`, `cancellation_policy_url`, `support_email` | **`catalog_variant_id=booking_complex`** + **`tool_name=modify_reservation`**; call detail `mapped_data` |
