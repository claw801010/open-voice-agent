...
| `insurance-fnol-faq` | FNOL guidance → **adjuster callback**; **quote intent** | `schedule_adjuster_callback`, `capture_quote_intent`, claims lookup tools (roadmap) | `scheduling_api_base_url`, `quoting_api_base_url`, `quote_product_code`, `carrier_name`, `line_of_business`, `claims_portal_url`, `preferred_callback_window_hours` | **`catalog_variant_id`** = `booking_complex` or `quote_complex`; **`tool_name`** filter + call detail `mapped_data` |
