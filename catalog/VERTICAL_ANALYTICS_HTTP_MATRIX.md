...
| `financial-services-banking-faq` | Card FAQ → **branch appointment**; **tokenized balance**; card block **roadmap** | `schedule_branch_appointment`, `lookup_account_balance` (shipped); `report_card_lost_stolen` (roadmap) | `scheduling_api_base_url`, `banking_api_base_url`, `institution_name`, `preferred_branch_code`, `branch_locator_url`, `card_block_portal_url`, `support_phone` | **`catalog_variant_id`** = `booking_complex` or `balance_lookup_complex`; **`tool_name`** filter + call detail `mapped_data` |
