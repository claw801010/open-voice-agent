...
| `hospitality-travel-concierge` | Concierge FAQ → **modify**; **waiver**; **loyalty upgrade** | `modify_reservation`, `apply_cancellation_waiver`, `offer_room_upgrade` | `pms_api_base_url`, `policy_api_base_url`, `crs_api_base_url`, `waiver_policy_code`, `upgrade_room_type`, `property_name`, `confirmation_prefix`, `default_room_type`, `cancellation_policy_url`, `support_email` | **`catalog_variant_id`** = `booking_complex`, `waiver_complex`, or `upsell_complex`; **`tool_name`** filter + call detail `mapped_data` |
| `financial-services-banking-faq` | Card FAQ → **branch appointment**; **tokenized balance**; **card block** | `schedule_branch_appointment`, `lookup_account_balance`, `report_card_lost_stolen` | `scheduling_api_base_url`, `banking_api_base_url`, `cards_api_base_url`, `card_block_reason_code`, `institution_name`, `preferred_branch_code`, `branch_locator_url`, `card_block_portal_url`, `support_phone` | **`catalog_variant_id`** = `booking_complex`, `balance_lookup_complex`, or `card_block_complex`; **`tool_name`** filter + call detail `mapped_data` |
| `smb-franchise-location-faq` | Multi-location FAQ → **lead callback**; **location router**; **CRM lead capture** | `schedule_lead_callback`, `route_call_to_location`, `capture_lead_intent` | `scheduling_api_base_url`, `locations_api_base_url`, `crm_api_base_url`, `crm_lead_source_code`, `routing_policy_code`, `brand_name`, `default_location_code`, `location_directory_url`, `preferred_callback_window_hours`, `support_phone` | **`catalog_variant_id`** = `booking_complex`, `location_router_complex`, or `lead_capture_complex`; **`tool_name`** filter + call detail `mapped_data` |
| `telecom-utilities-outage-faq` | Outage FAQ → **field callback**; **outage status**; **payment redirect** | `schedule_service_callback`, `lookup_outage_status`, `confirm_payment_redirect` | `scheduling_api_base_url`, `oss_api_base_url`, `billing_api_base_url`, `payment_redirect_reason_code`, `utility_name`, `default_service_area_code`, `outage_status_portal_url`, `payment_portal_url`, `support_phone` | **`catalog_variant_id`** = `booking_complex`, `outage_status_complex`, or `payment_redirect_complex`; **`tool_name`** filter + call detail `mapped_data` |
| `public-sector-civic-services-faq` | Civic FAQ → **case worker callback**; **permit status**; **multilingual routing** | `schedule_civic_callback`, `lookup_permit_status`, `route_by_language` | `scheduling_api_base_url`, `records_api_base_url`, `routing_api_base_url`, `language_routing_policy_code`, `supported_language_codes`, `agency_name`, `default_department_code`, `services_directory_url`, `support_phone` | **`catalog_variant_id`** = `booking_complex`, `permit_status_complex`, or `language_router_complex`; **`tool_name`** filter + call detail `mapped_data` |
| `hr-staffing-recruiting-faq` | Careers FAQ → **interview scheduling** (+ `.ics` invite); **application status**; **confirm / reschedule** | `schedule_interview`, `lookup_application_status`, `confirm_or_reschedule_interview` | `scheduling_api_base_url`, `ats_api_base_url`, `company_name`, `careers_portal_url`, `default_requisition_code`, `interview_type_default`, `preferred_interview_window_hours`, `support_phone` | **`catalog_variant_id`** = `booking_complex`, `application_status_complex`, or `interview_confirm_complex`; **`tool_name`** filter + call detail `mapped_data` (include `invite_download_url` when using local scheduling) |
