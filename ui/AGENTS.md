...
**Template catalog:** [e2e/catalog-marketplace.spec.ts](e2e/catalog-marketplace.spec.ts) — OSS **`/workflow/catalog` → /auth/login**; authenticated **Template catalog** + **buyer-demo** hint strips on all 10 packs; install/try/LoopTalk default to buyer **`catalog_variant_id`**; voice preview hosted audio + silent-placeholder hint. With **`E2E_EMAIL`** / **`E2E_PASSWORD`**: install → **Customize** → wire-local flows (skipped when **`E2E_EXPECT_STACK_AUTH=1`**).

**Buyer-demo analytics calls:** [e2e/analytics-buyer-demo-calls.spec.ts](e2e/analytics-buyer-demo-calls.spec.ts) — shared env map in [buyerDemoSeededCalls.ts](../src/lib/catalog/buyerDemoSeededCalls.ts); when CI seeds **`E2E_GTM_*_CALL_ID`**, asserts **Calls** list filter + **Call trace & quality** on detail for all ten verticals.

**GTM deck screenshots (optional):** set **`E2E_GTM_DECK_SCREENSHOTS=1`** with the same auth envs; run from **`ui/`**. Writes **41** frames under **`../docs/images/`** (`gtm-mk01-*`, `gtm-we01-*`) at **1280×720** — includes **Template catalog marketplace**, per-vertical **call-detail**, wire-local editor shots. Preflight: [gtm_live_capture_ready.sh](../scripts/gtm_live_capture_ready.sh). Batch seed: [seed_gtm_all_buyer_demo_calls.py](../scripts/seed_gtm_all_buyer_demo_calls.py), [seed_gtm_all_buyer_workflows.py](../scripts/seed_gtm_all_buyer_workflows.py). See [e2e/gtm-deck-screenshots.spec.ts](e2e/gtm-deck-screenshots.spec.ts).
