#!/usr/bin/env bash
# WE-01-VISUAL-DEPTH — one-shot authenticated Lighthouse on /overview + /reports.
#
...
#   LIGHTHOUSE_UI_ORIGIN=http://localhost:3000 npm run perf:lighthouse:auth:operator:full
#
...
npm run perf:lighthouse:auth:operator
npm run perf:lighthouse:summary:latest-operator
echo ""
echo "==> Paste the two WE-01-VISUAL-DEPTH lines into READMENEWRELEASES.md (Unreleased), then [x] the operator-shell Lighthouse item in READMEPLANTOEXECUTE.md."
